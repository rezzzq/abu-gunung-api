...
export function initLocationCheck(opts: LocationCheckOptions): LocationCheck {
  ...
  const render = (): void => {
    ...
    result.innerHTML = `<strong>${escapeHtml(verdict)}</strong>${escapeHtml(t(locale, "distanceFrom", { km }))}<br><span class="hint">${escapeHtml(t(locale, "forecastCaveat"))}</span>`;
  };
